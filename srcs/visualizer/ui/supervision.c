...
static void	create_supervision_panel_text_lines(t_visualizer *v, t_panel *panel, t_supervised_infos target_infos)
{
	...
	add_text_line(v->sdl.text_engine, v->sdl.font, panel, "Target Info : ", color_from_u32(0xFF000000), JUSTIFY_CENTER, 0);
	add_text_line(v->sdl.text_engine, v->sdl.font, panel, "Position : ", color_from_u32(0xFF000000), JUSTIFY_LEFT, 1);
	char *str = "(";
	str = ft_strjoin_free(str, ft_itoa(target_infos.pos.x), 2);
	str = ft_strjoin_free(str, ", ", 1);
	str = ft_strjoin_free(str, ft_itoa(target_infos.pos.y), 3);
	str = ft_strjoin_free(str, ")", 1);
	add_text_line(v->sdl.text_engine, v->sdl.font, panel, str, color_from_u32(0xFF000000), JUSTIFY_RIGHT, 1);
	free(str);

	add_text_line(v->sdl.text_engine, v->sdl.font, panel, "Team : ", color_from_u32(0xFF000000), JUSTIFY_LEFT, 2);
	str = ft_itoa(target_infos.team);
	add_text_line(v->sdl.text_engine, v->sdl.font, panel, str, get_team_color(target_infos.team), JUSTIFY_RIGHT, 2);
	free(str);

	add_text_line(v->sdl.text_engine, v->sdl.font, panel, "Alive : ", color_from_u32(0xFF000000), JUSTIFY_LEFT, 3);
	...
	add_text_line(v->sdl.text_engine, v->sdl.font, panel, str, alive_color, JUSTIFY_RIGHT, 3);
}
